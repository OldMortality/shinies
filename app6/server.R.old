...
## change log
##   11/11/2019   moved updates of reactive out of loop in 
##                  observeEvent(input$sample10  and 100
##                  This way, we update the tricky plot only once
##                Moved creation of basic top plot to global variable
##                  Perhaps it will speed things up a little.   
...
shinyServer <- function(input, output) {
  ...
  observeEvent(input$sample,{
    ...
    values$total <- c(values$total,thisSampleMean) 
    #s <- sd
    #lo <- meansamp - 1.96 * s/sqrt(as.numeric(input$n))
    #up <- meansamp + 1.96 * s/sqrt(as.numeric(input$n))
    
    s <- sqrt(var(samp))
    lo <- thisSampleMean + qt(0.025,as.numeric(input$n)-1) * s/sqrt(as.numeric(input$n))
    up <- thisSampleMean + qt(0.975,as.numeric(input$n)-1) * s/sqrt(as.numeric(input$n))
    if (! ((mu>lo) & (mu<up)) ) {
  ...
  observeEvent(input$sample10,{
    # moved update of lo and up (on which the tricky plot depends)
    #   out of the loop. This way, we update the tricky plot only once 
    totalValues <- vector()
    lo <- vector()
    up <- vector()
    for (i in 1:10) {
      ...
      meansamp <- round(mean(samp),2)
      thisSampleMean <- meansamp
      totalValues[i] <- thisSampleMean
      s <- sqrt(var(samp))
      lo[i] <- thisSampleMean + qt(0.025,as.numeric(input$n)-1) * s/sqrt(as.numeric(input$n))
      up[i] <- thisSampleMean + qt(0.975,as.numeric(input$n)-1) * s/sqrt(as.numeric(input$n))
      if (! ((mu>lo[i]) & (mu<up[i])) ) {
        countReds$counter <- countReds$counter + 1
      }
      #up <- meansamp + 1.96 * s/sqrt(as.numeric(input$n))
    ...
    samp(samp)
    values$total <- c(values$total,totalValues) 
  ...
  observeEvent(input$sample100,{
    ...
    # update the reactives out of the loop
    samp(samp)
    meansamp(meansamp) 
    values$total <- c(values$total,totalValues) 
  ...
  #
  # moved the creation of the basic plot out of the reactive bit
  #  
  ...
  output$plot1 <- renderPlot({
    ...
    if (length(samp())>0) {
      ...
      p <- topPlot + geom_point(data=pts,aes(y=y),
                                colour='black')
    } else {
      p <- topPlot
    }
