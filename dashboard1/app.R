...
ui <- dashboardPage(
  ...
  dashboardSidebar(useShinyjs(),
    ...
    sliderInput("xbar","Mean",-5,5,0),
    sliderInput("SD","Standard Deviation",0,10,1), 
    ...
    fluidRow(
      ...
      column(width=6,
        ...
        box(title="Means of all samples", 
          width=NULL,
          plotOutput("CLTplot3", height = 250)),
        box(
          width=NULL,
          title="All samples", 
...
server <- function(input, output) {
  ...
  output$CLTplot3 <- renderPlot({
    ...
    xbar <- round(1000* sum / input$n)/1000
